...
全库将 HTTP 栈从 `httpx` 迁移到 `httpx2`:运行时代码(`novon.core.http` 的 base client / feign)、测试工具(`novon-test-utils` 的 helpers / mock_router)统一 `import httpx2`;根 `[tool.uv] constraint-dependencies` 声明 `httpx2>=2.12`,成员按 [0003](./0003-centralized-version-constraints-at-workspace-root.md) 只写 bare name;移除 `httpx` 与 `respx` 依赖。
