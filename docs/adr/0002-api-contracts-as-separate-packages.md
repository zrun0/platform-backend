...
每个暴露 HTTP API 的服务配一个同级契约包 `packages/<service>-api/`(发行名 `novon-<service>-api`,模块名 `novon.<service>_api`),恰好包含三件事:
...
契约包只依赖 `novon-core`,绝不依赖 provider 的 app 包;consumer(BFF)依赖契约包而非服务包。
...
- 契约包不得长出业务逻辑:超出三件套的代码属于 `novon-core` 或 provider(review 把关)
