...
在 `novon-test-utils` 内自建 `MockRouter`——一个 `httpx2.AsyncBaseTransport` 子类,提供 respx 兼容 API 子集(`router.get/post(url)`、`route.return_value`、`route.called`、`route.calls.last.request`),经 `BaseServiceClient` 新增的 `transport=` 注入 seam 接入 client。未注册的请求抛 `AssertionError`。
...
- `MockRouter` 驻留 `novon-test_utils.mock_router`,经包根 `__init__` 导出
...
- [0001 - uv workspace monorepo + PEP 420 `novon.*` namespace](./0001-uv-workspace-monorepo-with-pep420-namespace.md)
