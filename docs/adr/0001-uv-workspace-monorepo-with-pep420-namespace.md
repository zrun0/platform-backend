# 0001 - uv workspace monorepo + PEP 420 `novon.*` namespace
...
采用 uv workspace monorepo,两层布局:可部署服务在 `apps/`(bff、uc、flow),共享库在 `packages/`(core、auth、契约包)。所有 import 代码位于 `novon.*` PEP 420 隐式命名空间(implicit namespace)下——命名空间根没有 `__init__.py`——每个发行包通过 `tool.uv.build-backend.module-name` 映射到一个模块(如 `novon-uc-api` → `novon.uc_api`)。

Why:

- 服务独立部署,共享代码经 `workspace = true` source 直连,无需私有 index 发布
- 单 lockfile/venv:跨包重构单 PR 完成,工具链(测试、类型检查)天然全树覆盖
- PEP 420 让任意发行包持有 `novon.*` 下任意模块,无需命名空间属主包
...
- 每个 app 只声明自己 import 的依赖(BFF 拉 `novon-uc-api` 而非 UC app),意外耦合在 type-check 阶段暴露
...
- `novon-test-utils` 例外:发行顶层模块 `novon_test_utils`(不在 `novon.*` 下),测试脚手架刻意置于运行时命名空间之外
